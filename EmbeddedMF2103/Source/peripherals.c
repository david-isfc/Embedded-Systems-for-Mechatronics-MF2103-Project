...
void Peripheral_PWM_ActuateMotor(int32_t vel)
{
	...
	if (vel > 0)
	{
			...
			if (duty_cycle > (int32_t)arr) duty_cycle = (int32_t)arr;
...
int32_t Peripheral_Encoder_CalculateVelocity(uint32_t ms)
{
    ...
    // -------------------------------------------------------------------------
    // Read counts since last reset (signed) and fix sign
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Simple IIR low-pass filter: alpha = 1/4 (tuneable)
    // rpm_filt[k] = rpm_filt[k-1] + alpha * (rpm - rpm_filt[k-1])
    // -------------------------------------------------------------------------
    {
        const int32_t alpha_num = 1;  // alpha = 1/4
        const int32_t alpha_den = 10;

        rpm_filt = (alpha_num * rpm +
                    (alpha_den - alpha_num) * rpm_filt) / alpha_den;
    }
